...
namespace FOW
{
    ...
    public class FogOfWarWorld : MonoBehaviour
    {
        ...
        {
            if (FogOfWarMaterial != null)
                FogOfWarMaterial.SetInt(numCirclesID, _numRevealers);
            if (FowTextureMaterial != null)
                FowTextureMaterial.SetInt(numCirclesID, _numRevealers);
            foreach (PartialHider hider in PartialHiders)
                hider.HiderMaterial.SetInt(numCirclesID, _numRevealers);
        }
        ...
        private CircleStruct[] _circleDataToSet = new CircleStruct[1];
        public void UpdateCircle(int id, CircleStruct data, int numHits, float[] radii, float[] distances, bool[] hits)
        {
            ...
            _circleDataToSet[0] = data;
            CircleBuffer.SetData(_circleDataToSet, 0, id, 1);
        ...
        {
            for (int i = 0; i < instance._numRevealers; i++)
            {

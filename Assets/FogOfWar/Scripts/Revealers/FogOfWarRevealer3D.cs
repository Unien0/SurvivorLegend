...
namespace FOW
{
	public class FogOfWarRevealer3D : FogOfWarRevealer
	{
        ...
        {
			return CachedTransform.position + FogOfWarWorld.UpVector * EyeOffset;
		}
		...
		protected override void _RevealHiders()
		{
			...
			{
				hiderInQuestion = FogOfWarWorld.HidersList[i];
                ...
                #region old code. remove in future
                //bool seen = false;
                ...
                //			if (!Physics.Raycast(EyePosition, hiderPosition - EyePosition, distToHider, ObstacleMask))
                //			{
                //				seen = true;
                //				break;
                //			}
        ...
        protected override bool _TestPoint(Vector3 point)
        {
			...
			if (distToPoint < UnobscuredRadius || (distToPoint < sightDist && Mathf.Abs(AngleBetweenVector2(point - EyePosition, ForwardVectorCached)) < ViewAngle / 2))
			{
				...
				if (!Physics.Raycast(EyePosition, hiderPosition - CachedTransform.position, distToPoint, ObstacleMask))
					return true;
			}
			return false;
		}
